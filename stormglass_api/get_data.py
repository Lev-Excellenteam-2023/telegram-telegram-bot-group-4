...
def get_bio (start_unix, end_unix):
    for area in FARM_AREA:
     ...
     'https://api.stormglass.io/v2/bio/point', ['soilMoisture', 'soilMoisture10cm','soilMoisture40cm' ,
    'soilMoisture100cm', 'soilTemperature', 'soilTemperature10cm','soilTemperature40cm','soilTemperature100cm'])
...
def get_weather ( start_unix, end_unix ):
    for area in FARM_AREA:
        json_data = get_data(FARM_AREA[area]["lat"], FARM_AREA[area]["lng"], start_unix, end_unix,
       'https://api.stormglass.io/v2/weather/point', ['airTemperature', 'pressure', 'cloudCover', 'currentSpeed',
       'humidity', 'windSpeed'])
